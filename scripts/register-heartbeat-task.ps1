...
#
# Assumes heartbeat-wrapper.ps1 has been copied to your own $env:USERPROFILE\.claude\
# directory (Task Scheduler needs a stable, non-repo path to invoke).

$wrapperPath = Join-Path $env:USERPROFILE ".claude\heartbeat-wrapper.ps1"
...
Register-ScheduledTask -TaskName "ToolRegistryHeartbeat" -Action $action -Trigger @($trigger1, $trigger2) -Settings $settings -Description "Checks Pieces activity against tool_registry.md and notifies of relevant tool suggestions. Exits immediately if Pieces OS isn't running." -Force

Write-Output "Task registered. It will run at next logon and every 45 minutes thereafter."
Write-Output 'To remove it later, run: Unregister-ScheduledTask -TaskName "ToolRegistryHeartbeat" -Confirm:$false'
